namespace Ikigai.Compiler.AST.Babel
...
type ArrowFunctionExpression(``params``, body, ?async_, ?loc) =
    ...
    let async = defaultArg async_ false
    let expression = match body with Choice1Of2 _ -> false | Choice2Of2 _ -> true
...
type ClassDeclaration(body, id, ?superClass, ?loc) =
...
type ClassExpression(body, ?id, ?superClass, ?loc) =
